...
export default function AdminManualSubjective() {
    ...
    const [rubricHindi, setRubricHindi] = useState("");
    ...
    const handleSave = async () => {
        if (!selectedSubjectId || questionText.trim() === "") {
            alert("Please fill out required Chapter and Question details.");
            return;
        }
        ...
        if (!rubricText.trim()) {
            alert("Subjective questions require a grading rubric.");
            return;
        }
        ...
        <div className="min-h-screen bg-slate-50 p-4 md:p-8">
            <div className="max-w-4xl mx-auto pb-32">
                <div className="flex items-center gap-4 mb-8">
                    ...
                    <h1 className="text-3xl font-black text-slate-900">Add Subjective</h1>
                </div>
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-4 mb-4">
                        {STATIC_SUBJECTS.map((sub) => (
                            <button
                                ...
                                className={`px-5 py-3 rounded-2xl border-2 font-bold whitespace-nowrap transition-colors ${selectedSubjectId === sub.id ? "border-amber-500 bg-amber-50 text-amber-600" : "border-slate-200 bg-white text-slate-500"}`}
                            >
                                {sub.name}
                            </button>
                        ))}
                ...
                <div className="bg-white border-2 border-slate-200 rounded-3xl p-6 md:p-8 shadow-sm mb-8">
                    <h2 className="text-lg font-bold text-slate-800 mb-6">2. Subjective Data</h2>

                    <h3 className="text-slate-700 font-bold mb-2">The Core Prompt</h3>
                    <textarea
                    ...
                    <div className="flex items-center justify-between mb-2">
                        <h3 className="text-slate-700 font-bold">Grading Rubric Points</h3>
                        <span className="text-xs text-slate-400 font-bold uppercase tracking-wider">One bullet per line</span>
                    </div>
                    <textarea
                        className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-4 font-medium text-slate-900 mb-4 min-h-[160px] focus:outline-none focus:border-blue-500 resize-y"
                        placeholder="1 Point for exact formula&#10;0.5 Points for intermediate step"
                        value={rubricText}
                        onChange={(e) => setRubricText(e.target.value)}
                    />
                    <textarea
                        className="w-full bg-blue-50 border border-blue-200 rounded-2xl px-5 py-4 font-medium text-slate-900 min-h-[160px] focus:outline-none focus:border-blue-500 resize-y"
                        placeholder="[Optional Hindi] 1 Point for exact formula..."
                        value={rubricHindi}
                        onChange={(e) => setRubricHindi(e.target.value)}
                    />
                </div>
                ...
                <button
                    onClick={handleSave}
                    disabled={loading}
                    className={`w-full flex items-center justify-center gap-3 py-5 rounded-2xl shadow-xl transition-all ${loading ? "bg-amber-400 cursor-not-allowed" : "bg-amber-600 hover:bg-amber-700 hover:-translate-y-1 shadow-amber-600/30"}`}
                >
                    {loading ? (
                        <div className="w-6 h-6 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
                    ) : (
                        <>
                            <span className="text-white font-black text-xl tracking-wide uppercase">Deploy Subjective</span>
